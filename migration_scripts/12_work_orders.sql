...
truncate table {db_new}.work_orders;

UPDATE {db_old}.jtktf01
SET reqdate = '1990-01-01'
WHERE reqdate < '1970-01-01';

INSERT INTO {db_new}.work_orders (id, dispatched, site_id, sales_rep_id, Invoice_ID, status_id, is_recurring, type,
...
SELECT ROW_NUMBER()                                                                                                OVER(ORDER BY jk.custnum ASC) AS id, 1 AS dispatched,
       pf.id                                                                                                    AS site_id,
       COALESCE((SELECT id FROM {db_new}.code_sets cs WHERE code = jk.tktsale AND parent_id = 117 LIMIT
       ...
       COALESCE((SELECT id FROM {db_new}.code_sets WHERE code = jk.prodtext AND parent_id = 107 LIMIT
                1), 107)                                                                                        AS status_id,
       1                                                                                                        AS is_recurring,
       COALESCE(
       ...
       1                                                                                                        AS processed,
       COALESCE((SELECT id FROM {db_new}.routes WHERE code = jk.grpcode LIMIT 1), NULL)                           AS route_id,
       ...
       COALESCE((SELECT id FROM {db_new}.code_sets WHERE code = jk.driver AND parent_id = 115 LIMIT
                1), NULL)                                                                                       AS driver_id,         -- driver blank in code_sets mapping
       COALESCE((SELECT id FROM {db_new}.code_sets WHERE code = jk.truck AND parent_id = 110 LIMIT
                1), NULL)                                                                                       AS truck_id,
...
FROM {db_old}.jtktf01 jk
         LEFT OUTER JOIN {db_new}.sites pf ON jk.custnum = pf.id;


UPDATE {db_new}.work_orders
SET stop_number = 999
...
ON {db_new}.assets.site_id = min_work_order.site_id
    SET {db_new}.assets.work_order_id = min_work_order.min_id;
