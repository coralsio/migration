INSERT INTO {db_new}.code_sets (code, description, value, parent_id)
SELECT DISTINCT
salecredit AS code,
...
FROM {db_old}.jrtf05 jr WHERE jr.rttypetkt IS NOT NULL and jr.rttypetkt <> ''
                 AND NOT EXISTS (
            SELECT 1
            FROM {db_new}.code_sets cs
            WHERE cs.parent_id = 111
...
FROM {db_old}.jrtf05 jr WHERE jr.rtratecode IS NOT NULL and jr.rtratecode <> ''
                 AND NOT EXISTS (
    SELECT 1
    FROM {db_new}.code_sets cs
...
FROM {db_old}.jtktf01 jt WHERE jt.ratecode1 IS NOT NULL and jt.ratecode1 <> ''
                  AND NOT EXISTS (
    SELECT 1
    FROM {db_new}.code_sets cs
    WHERE cs.parent_id = 106
...
FROM {db_old}.jtktf01 jt WHERE typesrv IS NOT NULL and typesrv <> ''
                  AND NOT EXISTS (
    SELECT 1
    FROM {db_new}.code_sets cs
    WHERE cs.parent_id = 111
...
FROM {db_old}.jcusf03 j3 WHERE j3.ntcode IS NOT NULL and j3.ntcode <> ''
                  AND NOT EXISTS (
    SELECT 1
    FROM {db_new}.code_sets cs
...
SELECT 'NOTE', 'NOTE', 'NOTE', 102
    WHERE NOT EXISTS (
    SELECT 1 FROM {db_new}.code_sets cs WHERE cs.code = 'NOTE' AND cs.parent_id = 102
);

INSERT INTO {db_new}.code_sets (code, description, value, parent_id)
SELECT distinct j8.ntcode as code, j8.ntcode as description,j8.ntcode as value,  118
FROM {db_old}.jcusf08 j8 WHERE j8.ntcode IS NOT NULL and j8.ntcode <> ''
                  AND NOT EXISTS (
    SELECT 1
    FROM {db_new}.code_sets cs
    WHERE cs.parent_id = 118
