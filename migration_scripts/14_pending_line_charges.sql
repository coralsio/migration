...
INSERT INTO {db_new}.line_charges (ID,
                                   ...
                                   total_amount,
                                   sales_rep_id)
SELECT ROW_NUMBER() OVER(ORDER BY jx.custnum ASC) +(SELECT max(id) FROM {db_new}.line_charges)AS ID, NULL AS work_order_service_id,
       ...
       CASE WHEN chrginv = 0 THEN NULL ELSE chrginv
END                      AS Invoice_id,
       pfs.customer_id                                                       AS customer_id,
       COALESCE(pfa.id, NULL)                                                AS asset_id,
       CASE WHEN COALESCE(chrgutype, '') <> '' THEN 'Asset' ELSE 'Other'
END AS type,
       ...
       CASE WHEN chrgtax > 0 THEN 1 ELSE 0
END                               AS is_taxable,
