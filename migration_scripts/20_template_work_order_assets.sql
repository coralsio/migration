...
SELECT DISTINCT pf.id AS                                                                                      template_work_order_id,
                ...
                IFNULL(
                            (SELECT id FROM {db_new}.pricing_templates WHERE name = jv.serial LIMIT 1), 777) AS pricing_template_id,
                1 AS                                                                                          quantity,
                1 AS                                                                                          is_assigned,
                ...
                COALESCE(
                        (SELECT id FROM {db_new}.code_sets c WHERE c.code = PFservice_rate_code_id),
                        NULL
                ...
                COALESCE(
                            (SELECT id FROM {db_new}.code_sets c WHERE c.code = PFcode6_rate_code_id),
                            NULL
                ...
                COALESCE(
                        (SELECT id FROM {db_new}.code_sets c WHERE c.code = PFdamage_rate_code_id),
                        NULL
                    ) AS                                                                                      custom_2_rate_code_id,

                COALESCE(PFdamage_schedule, NULL) AS                                                          custom_2_schedule,
                PFdamage_rate AS                                                                              custom_2_rate
FROM {db_new}.template_work_orders pf
--          INNER JOIN jivtf01 jv ON pf.custnum = jv.custnum commented due arrow db
     INNER JOIN {db_old}.jivtf01 jv ON pf.site_id = jv.custnum -- added due arrow db

         LEFT OUTER JOIN {db_new}.assets pfa ON pfa.serial_no = jv.serial
WHERE jv.custnum > 0;
...
UPDATE pfpre pft
    JOIN {db_new}.code_sets cs
ON cs.id = pft.asset_type_id
    JOIN {db_new}.pricing_templates pfp ON cs.code = pfp.name
    SET pft.pricing_template_id = pfp.id;

-- UPDATE pfpre pft
--     JOIN {db_new}.code_sets cs
-- ON cs.id = pft.asset_type_id
--     JOIN {db_new}.pricing_templates pfp ON cs.code = LEFT (pfp.serial, 2) AND CHAR_LENGTH (pfp.serial) = 2
--     SET pft.pricing_template_id = pfp.id;  commented due arrow db

UPDATE pfpre pft
    JOIN {db_new}.code_sets cs
ON cs.id = pft.asset_type_id
    JOIN {db_new}.pricing_templates pfp ON cs.code = LEFT (pfp.name, 3) AND CHAR_LENGTH (pfp.name) = 3
    SET pft.pricing_template_id = pfp.id;
...
UPDATE pfpre pft
    JOIN {db_new}.code_sets cs
ON cs.id = pft.asset_type_id
    JOIN {db_new}.pricing_templates pfp ON cs.code = LEFT (pfp.name, 4) AND CHAR_LENGTH (pfp.name) = 4
    SET pft.pricing_template_id = pfp.id;
...
UPDATE pfpre pft
    JOIN {db_new}.code_sets cs
ON cs.id = pft.asset_type_id
    JOIN {db_new}.pricing_templates pfp ON cs.code = LEFT (pfp.name, 2)
    AND SUBSTRING(pfp.name, 3, 1) REGEXP '^[0-9]$'
    SET pft.pricing_template_id = pfp.id
WHERE pft.pricing_template_id = 777;

truncate table {db_new}.template_work_order_assets;

INSERT INTO {db_new}.template_work_order_assets (id,
